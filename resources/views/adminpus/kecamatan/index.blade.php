                ...
                <div class="card-body">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ ($kecamatans->currentPage() - 1) * $kecamatans->perPage() + $index + 1 }}</td>
                                <td>{{ $kecamatan->kode_kec }}</td>
                                <td>{{ isset($kecamatan->provinsi) ? $kecamatan->provinsi->nama_provinsi : 'Data tidak
                                    tersedia' }}
                                </td>
                                <td>{{ isset($kecamatan->kabKota) ? $kecamatan->kabKota->nama_kab_kota : 'Data tidak
                                    tersedia' }}
                                </td>
                                <td>{{ $kecamatan->nama_kecamatan }}</td>
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('kecamatan.edit', $kecamatan->kode_kec) }}"
                                            class="btn btn-warning btn-sm">
                                            <i class="ri-edit-2-fill"></i>
                                        </a>
                                        <form action="{{ route('kecamatan.destroy', $kecamatan->kode_kec) }}"
                                            method="POST"
